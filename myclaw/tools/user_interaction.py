"""Interactive tools for gathering decisions and input from the end user."""
...
class Option(TypedDict):
    """A single selectable option for an `AskUserQuestion` prompt."""
...
class Question(TypedDict):
    """A question posed to the user, with options and a slot for the answer."""

    question: str
    options: list[Option]
    answer: str  # to be filled in by user


@tool
class AskUserQuestionTool(ToolBase):
    """Ask the end user a multiple-choice question and return their answer."""
    ...
    def run(self, question: Question) -> Question:
        ...
        Usage notes:
        - One question per tool call, if you have multiple questions to ask,
          call this tool multiple times.
        - Users will always be able to select "Other" to provide custom text
          input.
        ...
        ExitPlanMode. If you need plan approval, use ExitPlanMode instead.

        Args:
            question: A Question object containing the question text and
                options for the user to choose from.
        Returns:
            The same Question object with the 'answer' field filled in based
